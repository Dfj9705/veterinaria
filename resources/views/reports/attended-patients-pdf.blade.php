<!DOCTYPE html>
<html lang="es">

    <head>
        <meta charset="UTF-8">
        <title>Pacientes atendidos</title>

        <style>
            body {
                font-family: sans-serif;
                font-size: 11px;
                color: #111827;
            }

            h1 {
                text-align: center;
                font-size: 18px;
                margin-bottom: 4px;
            }

            .subtitle {
                text-align: center;
                font-size: 11px;
                color: #4b5563;
                margin-bottom: 14px;
            }

            .summary {
                margin-bottom: 12px;
            }

            table {
                width: 100%;
                border-collapse: collapse;
            }

            th {
                background: #e5e7eb;
                font-weight: bold;
            }

            th,
            td {
                border: 1px solid #9ca3af;
                padding: 6px;
                vertical-align: top;
            }

            .text-center {
                text-align: center;
            }
        </style>
    </head>

    <body>
        <h1>Reporte de pacientes atendidos</h1>

        <div class="subtitle">
            Generado el {{ now()->format('d/m/Y H:i') }}
        </div>

        <div class="summary">
            <strong>Desde:</strong> {{ $filters['from'] ?? 'Todos' }}
            &nbsp; | &nbsp;
            <strong>Hasta:</strong> {{ $filters['to'] ?? 'Todos' }}
            &nbsp; | &nbsp;
            <strong>Total pacientes:</strong> {{ $patients->count() }}
        </div>

        <table>
            <thead>
                <tr>
                    <th>Mascota</th>
                    <th>Propietario</th>
                    <th>Especie</th>
                    <th>Raza</th>
                    <th>Consultas</th>
                    <th>Última atención</th>
                </tr>
            </thead>

            <tbody>
                @forelse ($patients as $record)
                    <tr>
                        <td>{{ $record->pet?->name }}</td>
                        <td>{{ $record->pet?->customer?->name }}</td>
                        <td>{{ $record->pet?->species?->name }}</td>
                        <td>{{ $record->pet?->breed?->name }}</td>
                        <td class="text-center">{{ $record->total_consultations }}</td>
                        <td class="text-center">
                            {{ \Carbon\Carbon::parse($record->last_attention)->format('d/m/Y H:i') }}
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="text-center">
                            No hay pacientes atendidos para los filtros seleccionados.
                        </td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    </body>

</html>